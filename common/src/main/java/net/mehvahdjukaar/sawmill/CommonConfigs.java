...
public class CommonConfigs {
    ...
    static {
        ...
        builder.icon("sawmill").push("general");
        SORT_RECIPES = builder.worldReload()
                .comment("Sort recipes following cretive tab order. " +
        ...
        GEN_MODE = builder.gameRestart().comment("""
        ...
        ALLOW_NON_BLOCKS = builder.affectsDynamicPacks().worldReload()
                .comment("Allow crafting non-block items")
                .define("allow_non_blocks", true);
        ALLOW_NON_VARIANTS = builder.affectsDynamicPacks().worldReload()
                .comment("Allows crafting non wood variant items (crafting table for example)")
                .define("allow_non_variant", true);
        MOD_BLACKLIST = builder.affectsDynamicPacks().worldReload()
                .comment("List of Mod ids you want to completely exclude from sawmill recipes. " +
        ...
        PLANKS_ONLY_ONE = builder.affectsDynamicPacks().worldReload()
                .comment("Makes so planks recipes can only craft items with 1 input plank. Disable to allow more recipes")
        ...
        SPECIAL_COSTS = builder.affectsDynamicPacks().worldReload()
                .comment("This is a map of wood object type to its cost in planks. Its used to add some discount to some special blocks." +
        ...
        MAX_DISCOUNT = builder.affectsDynamicPacks().worldReload()
                .comment("Maximum discount that sawmill will given when converting recipes. Unit is percentage of input item")
